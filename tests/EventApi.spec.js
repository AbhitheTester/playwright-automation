...
async function loginAndGoToEvents(page) {
    const lgi= new Login(page)
    lgi.loginportal("abhishektest@yopmail.com","Test@123");
...
test("Apimock",async ({page,request})=>{
...
const eventcard=await page.locator('[id="event-card"]');
await expect(eventcard.first()).toBeVisible()
console.log(await eventcard.count())
await expect(await eventcard.count()).toEqual(4)
const sandbox=await page.getByText(/sandbox holds up to/i)
expect(sandbox).toBeVisible()
expect(await sandbox.textContent()).toContain("9 bookings")
